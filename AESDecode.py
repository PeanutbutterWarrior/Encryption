...
def unsubBytes(state):
    for i, column in enumerate(state):
        for j, byte in enumerate(column):
            ...
            if len(hexByte) == 1:
                hexByte = '0' + hexByte
            state[i][j] = inverseSBoxLookup[int(hexByte[0], 16)][int(hexByte[1], 16)]
    return state
...
def unmixColumns(state):
    out = []
    const = [[14, 11, 13, 9], [9, 14, 11, 13], [13, 9, 14, 11], [11, 13, 9, 14]]
    for column in state:
        newColumn = []
...
inverseSBoxLookup = [[52, 0x9, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb], 
